...
public class Problem11 {
	...
	for(int i=0;i<a.length;i++)
	{
		for(int j=0;j<a[i].length;j++)
		{
			...
			if(j<=a[i].length-4 && i<=a.length-4)
			{
				a1=a[i][j]*a[i][j+1]*a[i][j+2]*a[i][j+3];
				a2=a[i][j]*a[i+1][j]*a[i+2][j]*a[i+3][j];
				a3=a[i][j]*a[i+1][j+1]*a[i+2][j+2]*a[i+3][j+3];
								
			}
			else if(j>a[i].length-4 && i<=a.length-4)
			{
				a1=0;
				a2=a[i][j]*a[i+1][j]*a[i+2][j]*a[i+3][j];
				a3=0;
